...
In their paper (cite), Miller et al. modified the \texttt{malloc} library function to return \texttt{NULL} with some probability indicting that memory allocation has failed. They then tested a variety of Linux utilities using this version of \texttt{malloc} to test if they correctly handled this error case. Surprisingly, the majority of these utilities did not correctly handle this case and crashed in a variety of ways. We plan to expand on this idea and test significantly more system/library calls in a similar way on more applications.

Furthermore, we hope to design and implement a tool that will allow for others to easily test their programs for poorly handled return values. In this end, we plan to design a way to automatically generate function wrappers (see Section 3) for a system/library call specified by the user.  
